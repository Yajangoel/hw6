...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), resizeAlpha_(resizeAlpha), totalProbes_(0), mIndex_(0), countItems(0), countDeleted(0)
{
...
{
    double load = double(countItems + countDeleted) / CAPACITIES[mIndex_];
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    size_t oldM = CAPACITIES[mIndex_];
    std::vector<HashItem*> oldTable = std::move(table_);
    mIndex_++;
    table_.assign(CAPACITIES[mIndex_], nullptr);
    countItems = countDeleted = 0;
    for(HashItem* temp : oldTable){
